...
-- Sowohl PUBLIC als auch die in Supabase separat berechtigte Rolle anon werden entzogen.

revoke execute on function public.admin_adjust_user_coins(bigint,bigint,text) from public, anon;
revoke execute on function public.admin_rename_company(bigint,text,text) from public, anon;
revoke execute on function public.admin_set_admin_role(bigint,text,text) from public, anon;
revoke execute on function public.admin_set_company_money(bigint,numeric,text) from public, anon;
revoke execute on function public.admin_set_user_premium(bigint,boolean,timestamp with time zone,text) from public, anon;
revoke execute on function public.admin_set_user_status(bigint,text,text) from public, anon;
revoke execute on function public.admin_unlock_user(bigint,text) from public, anon;
revoke execute on function public.get_player_messages(integer) from public, anon;
revoke execute on function public.mark_player_message_read(bigint) from public, anon;
revoke execute on function public.send_player_message(text,text,text) from public, anon;
revoke execute on function public.take_expansion_loan(bigint,numeric,numeric,integer,numeric) from public, anon;
revoke execute on function public.transfer_business_money(bigint,bigint,numeric) from public, anon;
