...
begin
  ...
  select u.id,u.public_id,u.username::text,u.email::text,u.display_name::text,u.status::text,
         coalesce(u.premium,false),u.premium_until,u.created_at,u.last_login_at,
         coalesce(u.failed_login_count,0),u.locked_until,s.role::text,coalesce(w.balance,0)
  from public.users u
  left join public.admin_staff s on s.user_id=u.auth_user_id and s.active=true
...
begin
  ...
  if v_after<0 then raise exception 'Coinstand darf nicht negativ werden'; end if;
...
begin
  ...
  select jsonb_build_object('premium',u.premium,'premium_until',u.premium_until) into v_before from public.users u where u.id=p_user_id for update;
  if v_before is null then raise exception 'Spieler nicht gefunden'; end if;
  if coalesce(p_enabled,false) and p_until is not null and p_until<=now() then raise exception 'Premium-Ende muss in der Zukunft liegen'; end if;
  update public.users u
    set premium=coalesce(p_enabled,false), premium_until=case when coalesce(p_enabled,false) then p_until else null end
    where u.id=p_user_id;
  select jsonb_build_object('premium',u.premium,'premium_until',u.premium_until) into v_after from public.users u where u.id=p_user_id;
  perform private.admin_audit('user.premium.set','user',p_user_id::text,p_reason,v_before,v_after);
  return query select u.id,u.premium,u.premium_until from public.users u where u.id=p_user_id;
end;
$$;
...
begin
  ...
  select c.money into v_before from public.companies c where c.id=p_company_id for update;
...
begin
  ...
  select jsonb_build_object('status',u.status) into v_before from public.users u where u.id=p_user_id for update;
  if v_before is null then raise exception 'Spieler nicht gefunden'; end if;
...
begin
  ...
  select jsonb_build_object('failed_login_count',coalesce(u.failed_login_count,0),'locked_until',u.locked_until,'status',u.status) into v_before
