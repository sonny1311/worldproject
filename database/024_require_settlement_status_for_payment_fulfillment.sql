-- Only transactions already submitted for settlement (or further along) may grant paid entitlements.

CREATE OR REPLACE FUNCTION public.fulfill_braintree_purchase(p_user_id bigint, p_provider_transaction_id text, p_sku text, p_amount_eur numeric, p_currency text, p_status text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
begin
 ...
 select * into v_product from public.store_products where sku=p_sku and active=true for share;
 ...
 on conflict(provider,provider_transaction_id) do nothing returning id into v_purchase_id;
 if v_purchase_id is null then return jsonb_build_object('success',true,'duplicate',true,'transactionId',p_provider_transaction_id); end if;
 if v_product.kind='coins' then
   insert into public.coin_wallets(user_id,balance,updated_at) values(p_user_id,v_product.coin_amount,now())
   on conflict(user_id) do update set balance=public.coin_wallets.balance+excluded.balance,updated_at=now() returning balance into v_balance;
 ...
 update public.users set premium_plan=v_product.premium_plan,
   premium_until=(greatest(coalesce(premium_until,now()),now()) + make_interval(days=>v_product.duration_days)),premium_auto_renew=false
 where id=p_user_id returning premium_until into v_until;
