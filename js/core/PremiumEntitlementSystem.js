...
export function runPremiumEntitlementTest(){const now=1000000,p=new PremiumEntitlementSystem(),a={premiumUntil:now+86400000};if(p.constructionLimit(a,now)!==5||p.productionQueueLimit(a,now)!==3||p.storageCapacity(a,10000,now)!==12000||!p.canUseGuidedSetupNavigation(a,now)||!p.canUseAutomaticStaffAssignment(a,now)||!p.canUseSmartDeliveryQuantity(a,now))throw new Error("Premiumvorteile fehlerhaft");a.premiumUntil=now-1;const expired=p.overCapacityState(a,{baseStorage:10000,currentStored:11500,runningConstruction:5,queuedProduction:3,now});if(!expired.storageOverfilled||!expired.constructionOverLimit||!expired.productionQueueOverLimit||p.storageCapacity(a,10000,now)!==10000||p.canUseGuidedSetupNavigation(a,now)||p.canUseAutomaticStaffAssignment(a,now)||p.canUseSmartDeliveryQuantity(a,now))throw new Error("Premium-Ablauflogik fehlerhaft");console.log("✅ PREMIUM: 3→5 BAU, +20% LAGER, 0→3 PRODUKTIONSQUEUE, DIREKTHILFE, PERSONALZUWEISUNG, LIEFERMENGENVORSCHLAG UND DATENERHALT BEI ABLAUF ERFOLGREICH",expired);return true;}
