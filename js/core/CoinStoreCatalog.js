...
export function runCoinStoreCatalogTest(){if(COIN_STORE_PACKAGES.length<8)throw new Error('Zu wenige Coin-Pakete');let lastValue=0;for(const p of COIN_STORE_PACKAGES){if(!(p.coins>0&&p.priceEuro>0))throw new Error('Ungueltiges Coin-Paket');const value=p.coins/p.priceEuro;if(value<lastValue)throw new Error('Groesseres Coin-Paket darf keinen schlechteren Coin-Wert haben');lastValue=value;}const r=beginCoinStoreCheckout('coins_100');if(!r.pendingPaymentIntegration)throw new Error('Coinshop darf ohne Payment keine Coins gutschreiben');return true;}
