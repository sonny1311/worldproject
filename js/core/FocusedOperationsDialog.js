...
export class FocusedOperationsDialog {
  ...
  renderPurchasing(panel,company){
    this.header(panel,"📦 Einkauf");panel.append(this.el("p","Hier werden ausschließlich Rohstoffe und neues Verpackungsmaterial bestellt."));
    const suppliers=suppliersForCompany(company),cycleStock=new Set(["used_bottles","dirty_bottles","clean_bottles","used_bottles_050","dirty_bottles_050","clean_bottles_050"]),materials=[...new Set(suppliers.flatMap(s=>s.materials||[]))].filter(id=>!cycleStock.has(id));
