...
export class AccountProfileDialog {
    ...
    async open(){if(this.overlay)return;const overlay=this.el("div");Object.assign(overlay.style,{position:"fixed",inset:0,zIndex:21000,background:"rgba(0,0,0,.72)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"});const panel=this.el("div");Object.assign(panel.style,{width:"min(620px,95vw)",maxHeight:"90vh",overflow:"auto",background:"#fff",color:"#111",borderRadius:"14px",padding:"22px",fontFamily:"Arial,sans-serif"});overlay.append(panel);this.parent.append(overlay);this.overlay=overlay;await this.render(panel);}
    close(){this.overlay?.remove();this.overlay=null;}

    async render(panel){
        panel.innerHTML="";let data;
        try{data=await this.api.accountOverview();}catch(error){panel.append(this.el("h2","Account"),this.el("div",`Serverprofil nicht verfügbar: ${error.message}`),this.button("Schließen",()=>this.close()));return;}
        const user=data.user||{},wallet=data.wallet||{balance:0},company=data.company||null,transactions=data.transactions||[];
        ...
        const display=this.input(user.display_name||user.username||"");const country=this.input(user.country_code||"DE");const language=this.input(user.language_code||"de");
        ...
        if(company){panel.append(this.el("div",`${company.name} · ${this.money(company.money)} € Spielgeld`),this.el("div",company.saved_at?`Letzter Spielstand: ${new Date(company.saved_at).toLocaleString("de-DE")}`:"Noch kein Spielstand gespeichert"));}
        else panel.append(this.el("div","Noch keine Firma gegründet."));
        panel.append(this.button("Abmelden",async()=>{await window.worldAccounts?.gameAccessGate?.logout?.();}));
    }
}
