...
export class AccountProfileDialog {
    ...
    async render(panel){
        ...
        if(this.isAdmin(user)){const badge=this.el('div',`🛠 Adminzugang: ${this.adminRole(user)}`);Object.assign(badge.style,{margin:'10px 0',padding:'9px 12px',borderRadius:'8px',background:'#fff4d6',border:'1px solid #d29922',fontWeight:'800'});panel.append(badge);}
        ...
        actions.push(this.button("Abmelden",async()=>{await window.worldAccounts?.gameAccessGate?.logout?.();}));panel.append(...actions);
        window.dispatchEvent(new CustomEvent('world:profile-updated',{detail:{user:window.worldCurrentUser||user}}));
    }
}
