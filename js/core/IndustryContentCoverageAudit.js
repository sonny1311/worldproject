...
export function auditIndustryContent(type){
 const p=IndustryProfiles[type],key=p.branchKey;
 ...
 const recipeMachines=recipes.map(x=>x.machineType).filter(Boolean),missingMachines=[...new Set(recipeMachines.filter(id=>!machineIds.has(id)))];
