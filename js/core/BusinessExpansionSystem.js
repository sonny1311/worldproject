...
// Eine weitere Betriebsgründung hat keine zusätzliche, versteckte Pauschalgebühr.
// Bezahlt wird die gewählte Immobilie (Miete/Kauf) über BusinessLocationSystem bzw. serverseitig dieselbe Preislogik.
export function expansionRequirements(existingBusinesses=0){if(existingBusinesses<=0)return {creationCost:0,requiredNetWorth:0,requiredCashReserve:0,requiredManagement:0};const n=existingBusinesses;return {creationCost:0,requiredNetWorth:Math.round(220000*Math.pow(1.62,n-1)),requiredCashReserve:Math.round(45000*Math.pow(1.55,n-1)),requiredManagement:n,administrationMultiplier:1+Math.max(0,n-1)*0.12};}
...
export function canExpand({businesses=[],sourceCompany=null,managementCapacity=0}={}){const req=expansionRequirements(businesses.length),cash=Number(sourceCompany?.money||0),netWorth=businesses.reduce((s,c)=>s+estimateCompanyNetWorth(c),0);const reasons=[];if(businesses.length>0&&cash<req.requiredCashReserve)reasons.push("Zu wenig Liquiditaetsreserve");if(netWorth<req.requiredNetWorth)reasons.push("Unternehmenswert noch zu niedrig");if(Number(managementCapacity||0)<req.requiredManagement)reasons.push("Management-Kapazitaet reicht nicht");return {allowed:reasons.length===0,reasons,requirements:req,cash,netWorth};}
...
export function runBusinessExpansionTest(){console.log("======================================");console.log("BETRIEBS-EXPANSIONS-/AUSBAUTEST");const start=1000000,c={money:1000000,upgrades:{}};const a=applyUpgrade(c,"production",{now:start}),b=applyUpgrade(c,"storage",{now:start});if(c.upgrades.production||c.upgrades.storage)throw new Error("Ausbau darf nicht sofort wirksam werden");if(!a.pending||!b.pending||!a.job?.finishAt||!b.job?.finishAt)throw new Error("Zeitbasierter Ausbauauftrag fehlt");const system=new BusinessUpgradeSystem();system.process(c,Math.max(a.job.finishAt,b.job.finishAt));if(c.upgrades.production!==1||c.upgrades.storage!==1)throw new Error("Fertiger Ausbau wird nicht aktiviert");const r2=expansionRequirements(1),r3=expansionRequirements(2);if(r2.creationCost!==0||r3.creationCost!==0)throw new Error("Versteckte Betriebsgründungsgebühr darf nicht zurückkehren");if(r3.requiredNetWorth<=r2.requiredNetWorth)throw new Error("Expansionsanforderungen steigen nicht");console.log("✅ BETRIEBS-EXPANSIONS-/AUSBAUTEST ERFOLGREICH",{a,b,r2,r3});return true;}
