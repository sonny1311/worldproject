...
export function branchKeyFor(companyOrIndustry){
 const raw=typeof companyOrIndustry==="string"?companyOrIndustry:(companyOrIndustry?.branchKey||companyOrIndustry?.branch_key||companyOrIndustry?.industry||companyOrIndustry?.type||companyOrIndustry?.company_type||"");
 const profile=worldContentRegistry.get("industries",raw);
 return profile?.branchKey||raw;
}
export function materialsForIndustry(industry=""){
 const key=branchKeyFor(industry),registered=worldContentRegistry.get("industries",industry);
 if(registered?.allowedItems?.length)return [...registered.allowedItems];
 return [...(IndustryMaterials[key]||[])];
}
export function suppliersForCompany(company){const key=branchKeyFor(company);return worldContentRegistry.list("suppliers",{filter:s=>(s.industries||[]).includes(key)});}
export function recipesForCompany(company){const key=branchKeyFor(company);return worldContentRegistry.list("recipes",{filter:r=>(r.industries||[]).includes(key)});}
export function productsForCompany(company){const key=branchKeyFor(company);return worldContentRegistry.list("products",{filter:p=>(p.industries||[]).includes(key)});}
export function supplierAllowed(company,supplier){const key=branchKeyFor(company);if((supplier?.industries||[]).length)return supplier.industries.includes(key);const allowed=new Set(materialsForIndustry(company));return (supplier?.materials||[]).some(x=>allowed.has(x));}
...
export class WarehouseSystem{
 ...
 zoneFor(material){return worldContentRegistry.get("materials",material)?.storageZone||(["bottles","caps","labels","packaging"].includes(material)?"packaging":"raw");}
...
export class ProductionPlanner{
 ...
 pause(jobId){const j=this.queue.find(x=>x.id===jobId);if(!j||j.status!=="running")return false;j.status="paused";j.machine.busy=false;return true;}
 resume(jobId,now=Date.now()){const j=this.queue.find(x=>x.id===jobId);if(!j||j.status!=="paused"||j.machine.busy)return false;j.status="running";j.machine.busy=true;j.finishAt=now+j.plan.durationMinutes*60000;return true;}
 cancel(jobId){const j=this.queue.find(x=>x.id===jobId);if(!j||["finished","cancelled"].includes(j.status))return false;j.status="cancelled";j.machine.busy=false;return true;}
 move(jobId,newIndex){const i=this.queue.findIndex(x=>x.id===jobId);if(i<0)return false;const [j]=this.queue.splice(i,1);this.queue.splice(Math.max(0,Math.min(newIndex,this.queue.length)),0,j);return true;}
...
export function runOperationalSupplyChainTest(){const company={id:1,industry:"brewery"},supplier=worldContentRegistry.get("suppliers","brew_malt_regional")||{id:"test",industries:["brewery"],materials:["malt"],prices:{malt:1},distanceKm:30,deliveryBase:20,deliveryPerKm:.5,deliveryHours:2,quality:.95,reliability:.98};const contracts=new SupplierContractSystem(),orders=new SupplyOrderSystem({contracts}),warehouse=new WarehouseSystem(),contract=contracts.create({supplierId:supplier.id,material:"malt",minQuantity:500,discount:.03}),vehicle={id:7,payloadKg:12000,volumeM3:40,palletCapacity:18};const order=orders.createOrder({company,supplier,material:"malt",quantity:500,transportMode:"own",vehicle,cargo:{weightKg:500,volumeM3:2,pallets:1},now:0});orders.advance(order.eta);warehouse.receive(order);const planner=new ProductionPlanner({warehouse,machines:[{id:1,type:"brewhouse",busy:false}]});const recipe={id:"beer",product:"beer",materials:{malt:100},machineType:"brewhouse",durationMinutes:60,output:1000,variableCost:80};const job=planner.start(recipe,1,0);planner.advance(job.finishAt);if(warehouse.stock.finished.beer!==1000||order.contractId!==contract.id||!suppliersForCompany(company).length)throw new Error("Supply-Chain-Test fehlgeschlagen");console.log("✅ OPERATIVER LIEFERKETTEN-/PRODUKTIONSTEST ERFOLGREICH",{order,job,orderSummary:orders.summary(),warehouse:warehouse.overview()});return true;}
