...
export function runSolarInvestmentTest(){const c={money:100000};buySolar(c,'pv_5',{now:1});buySolar(c,'pv_20',{now:2});if(c.solar.installedKwp!==25||c.solar.invested!==27500)throw new Error('Solaranlagen werden nicht additiv ausgebaut');if(c.financialLog.length!==2||c.financialLog[1].type!=='solar_investment'||c.costLedger.length!==2)throw new Error('Solarinvestition fehlt im Finanz-/Kostenledger');const q=electricityQuote(c,{demandKw:100,hours:1,gridPricePerKwh:.3,productionFactor:1});if(q.solarKwh!==25||q.gridKwh!==75||Math.abs(q.savedCost-7.5)>.001)throw new Error('Solarstrom reduziert Netzbezug nicht korrekt');return true;}
