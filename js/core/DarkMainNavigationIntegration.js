...
function styleNav(){
 ...
 for(const b of nav.querySelectorAll('button'))Object.assign(b.style,{background:'#1e293b',color:'#f8fafc',border:'1px solid #475569',borderRadius:'9px',padding:'9px 12px',boxShadow:'none',fontWeight:'800'});
 nav.dataset.worldDarkMainNav='1';return true;
}
export function installDarkMainNavigation(){if(typeof document==='undefined')return false;const apply=()=>styleNav();if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply);else apply();const observer=new MutationObserver(()=>{if(document.getElementById(NAV_ID)&&!document.getElementById(NAV_ID)?.dataset.worldDarkMainNav)styleNav();for(const id of obsoleteIds)document.getElementById(id)?.remove();});observer.observe(document.documentElement,{childList:true,subtree:true});window.addEventListener('world:access-granted',()=>setTimeout(apply,0));return true;}
