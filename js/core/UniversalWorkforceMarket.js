...
export function generateApplicants(company,{count=8,seed=Date.now()}={}){const jobs=jobsFor(company),out=[];for(let i=0;i<count;i++){const j=jobs[(Math.abs(Math.floor(seed/1000))+i)%jobs.length],quality=.65+((i*17+Math.floor(seed/10000))%35)/100,wage=Math.round(n(j.baseWageMonthly)*(0.9+quality*.18));out.push({id:`app-${seed}-${i}`,jobId:j.id,role:j.label,qualification:Math.round(quality*100)/100,wageMonthly:wage,skills:[...(j.skills||[])],available:true});}return out;}
...
export function runUniversalWorkforceTest(){const checks=[];for(const type of ["Brauerei","Schreinerei","Bäckerei","Metzgerei","Landwirtschaftsbetrieb","Tierhaltung","Obstbau","Maschinenbau","Metallverarbeitung","Kunststoffverarbeitung","Einzelhandel","Großhandel","Onlinehandel"]){const c={type,industry:"",employees:[]},jobs=jobsFor(c),apps=generateApplicants(c,{count:5,seed:10000});checks.push({name:`${type} jobs`,success:jobs.length>=3},{name:`${type} applicants`,success:apps.length===5});}const failed=checks.filter(x=>!x.success);return{success:!failed.length,checks,failed,total:checks.length,passed:checks.length-failed.length};}
