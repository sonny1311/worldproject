...
export const UniversalJobs=[...(WorkforceContent.jobs||[]),...GENERIC];
...
export function generateApplicants(company,{count=8,seed=Date.now()}={}){const jobs=jobsFor(company),out=[],required=requiredRolesForCompany(company);for(const role of required){const j=jobs.find(x=>x.id===role)||UniversalJobs.find(x=>x.id===role);if(j&&!out.some(x=>x.jobId===j.id))out.push(applicantForJob(j,{seed,index:out.length,required:true}));}if(!jobs.length)return out.slice(0,count);for(let i=0;out.length<count&&i<count*3;i++){const j=jobs[(Math.abs(Math.floor(seed/1000))+i)%jobs.length];if(!j)break;out.push(applicantForJob(j,{seed,index:out.length}));}return out.slice(0,Math.max(count,required.length));}
...
export function runUniversalWorkforceTest(){const checks=[];for(const type of ["Brauerei","Schreinerei","Bäckerei","Metzgerei","Landwirtschaftsbetrieb","Tierhaltung","Obstbau","Maschinenbau","Metallverarbeitung","Kunststoffverarbeitung","Einzelhandel","Großhandel","Onlinehandel"]){const c={type,industry:"",employees:[]},jobs=jobsFor(c),apps=generateApplicants(c,{count:5,seed:10000});checks.push({name:`${type} jobs`,success:jobs.length>=3},{name:`${type} applicants`,success:apps.length>=5});if(type==="Brauerei")checks.push({name:"Brauerei Braumeister verfügbar",success:apps.some(a=>a.jobId==="brew_master")});}const failed=checks.filter(x=>!x.success);return{success:!failed.length,checks,failed,total:checks.length,passed:checks.length-failed.length};}
