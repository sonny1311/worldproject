...
export function mountBusinessQuickSwitcher(){
 ...
 async function render(){try{await businessPortfolio.refresh();}catch{return;}const list=sortCompanies(businessPortfolio.companies);host.innerHTML='';if(!list.length){host.style.display='none';return;}host.style.display='flex';let idx=list.findIndex(c=>idOf(c)===activeId());if(idx<0)idx=0;const current=list[idx]||list[0],prev=list[(idx-1+list.length)%list.length],next=list[(idx+1)%list.length];if(current)acknowledge(current);
  ...
  const rightA=unseenAttention(next),right=btn(`${next?.name||'Nächster Betrieb'}${rightA.needsAttention?' 🔔'+rightA.count:''} ▶`,next?.name||'Nächster Betrieb');right.disabled=list.length<2;right.onclick=()=>switchTo(next,rightA.needsAttention);if(rightA.needsAttention){right.style.borderColor='#fbbf24';right.style.boxShadow='0 0 14px rgba(251,191,36,.55)';}if(rightA.unseen)right.style.animation='worldBusinessPulse .65s ease-in-out 3';
  host.append(left,center,right);
  const attention=list.filter(c=>idOf(c)!==activeId()).map(c=>({c,a:unseenAttention(c)})).filter(x=>x.a.needsAttention);if(attention.length){const total=attention.reduce((s,x)=>s+x.a.count,0),bell=btn(`🔔 ${total}`,`${attention.length} Betriebe brauchen Aufmerksamkeit`);bell.onclick=()=>switchTo(attention[0].c,true);host.append(bell);}
 }
