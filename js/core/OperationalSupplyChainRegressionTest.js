...
export function runOperationalSupplyChainRegressionTest(){
 ...
 const warehouse=new WarehouseSystem({raw:1000,packaging:1000,finished:1000,cold:1000});
 ...
 check("11 Lagergrenze wird eingehalten",()=>{const full={status:"arrived",material:"malt",quantity:501};expectThrow(()=>warehouse.receive(full),"Ueberfuellung wurde akzeptiert");assert(warehouse.stock.raw.malt===500,"Ueberfuellung hat Bestand veraendert");});
 ...
 check("14 Reload erzeugt keine zweite Gutschrift",()=>{const restoredOrder=clone(order),restoredWarehouse=new WarehouseSystem({raw:1000,packaging:1000,finished:1000,cold:1000});restoredWarehouse.stock=clone(warehouse.stock);expectThrow(()=>restoredWarehouse.receive(restoredOrder),"Reload erlaubte doppelte Einlagerung");assert(restoredWarehouse.stock.raw.malt===500,"Reload verdoppelte Bestand");});
 ...
 warehouse.stock.finished.beer_bulk_pils=500;
 ...
 console.log("✅ OPERATIONAL-SUPPLY-CHAIN-REGRESSION 25/25 ERFOLGREICH",{checks});
