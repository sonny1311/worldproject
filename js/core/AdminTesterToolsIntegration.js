...
AdminConsoleUI.prototype.playerDetail=function(player){
  ...
  if(player.premium==null){const until=player.premium_until||player.premiumUntil;player.premium=!!until&&new Date(until).getTime()>Date.now();}
  ...
  box.append(row('Aktuelle Coins',player.__adminWalletLoading?'wird geladen …':n(player.coins).toLocaleString('de-DE')),row('Premium bis',premiumUntil?new Date(premiumUntil).toLocaleString('de-DE'):'Kein Premium'));

  const coinBar=el('div');coinBar.className='toolbar';
  for(const amount of [100,500,1000,5000]){const b=btn(`+${amount.toLocaleString('de-DE')} Coins`,()=>withBusy(this,b,async()=>{const reason=`Tester-Gutschrift +${amount} Coins`;const balance=await persistPlayerCoins(player,{delta:amount,reason});player.coins=balance;player.__adminWalletLoaded=true;this.admin.log(this.actor,'tester_coins_credit',{playerId:id(player),amount,balance,reason});this.toast(`✅ ${amount.toLocaleString('de-DE')} Coins gutgeschrieben`);this.render();}));coinBar.append(b);}
