...
export function runPremiumDailyCoinTest(){const now=new Date(2026,7,14,10).getTime(),account={id:'test-user',premiumUntil:now+86400000,premiumPlan:'premium_plus'},company={coins:5};const a=grantPremiumDailyCoins(account,company,now),b=grantPremiumDailyCoins(account,company,now+1000);if(a.granted!==10||company.coins!==15||b.granted!==0||!company.accountEntitlementState?.premiumDailyCoinClaims?.[dayKey(now)])throw new Error('Premium-Plus-Tagescoins fehlerhaft');const restoredAccount={id:'test-user',premiumUntil:now+86400000,premiumPlan:'premium_plus'},restoredCompany=JSON.parse(JSON.stringify(company)),c=grantPremiumDailyCoins(restoredAccount,restoredCompany,now+2000);if(c.granted!==0)throw new Error('Premium-Plus-Tagescoins nach Reload doppelt gutgeschrieben');return true;}
