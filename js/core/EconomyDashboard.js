...
export class EconomyDashboard {
    ...
    label(id){if(!id)return "Artikel";const materialAliases={malt_kg:"malt",hops_kg:"hops",yeast_kg:"yeast",water_l:"water",bottle_033:"bottles",crown_cap:"caps",label_033:"labels"};const materialId=materialAliases[id]||id,translated=i18n.materialLabel(materialId);if(translated!==`materials.${materialId}`)return translated;return({lager033_bottle:"Lagerbier 0,33 l",pils033_bottle:"Pils 0,33 l",lager033:"Lagerbier",pils:"Pils",timber_spruce_m3:"Fichtenholz",timber_oak_m3:"Eichenholz",board_mdf_m2:"MDF-Platten",glue_kg:"Holzleim",fittings_set:"Beschlagsatz"})[id]||"Artikel";}
    productionStatus(status){const key=`status.production.${status||"queued"}`,text=i18n.t(key);if(text!==key)return text;const fallback=i18n.t("status.production.queued");return fallback===key?"Geplant":fallback;}
    orderStatus(status){const key=`status.order.${status||"open"}`,text=i18n.t(key);if(text!==key)return text;const fallback=i18n.t("status.order.open");return fallback==="status.order.open"?"Offen":fallback;}
    vehicleStatus(status){const known={available:"Verfügbar",idle:"Verfügbar",busy:"Im Einsatz",maintenance:"In Wartung",broken:"Defekt",sold:"Verkauft"};return known[status]||"Verfügbar";}
    ...
    openStatusOverlay(type){
        const shade=this.el("div");Object.assign(shade.style,{position:"fixed",inset:0,zIndex:24000,background:"rgba(0,0,0,.78)",display:"flex",alignItems:"center",justifyContent:"center",padding:"18px"});
        const box=this.el("div");Object.assign(box.style,{width:"min(980px,95vw)",maxHeight:"90vh",overflow:"auto",background:"#fff",color:"#111",borderRadius:"14px",padding:"18px",fontFamily:"Arial,sans-serif"});
        const head=this.el("div");Object.assign(head.style,{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"10px"});const title=type==="deliveries"?"🚚 Transportstatus":"🏗️ Produktionsstatus";head.append(this.el("h2",title),this.button("✕",()=>shade.remove()));box.append(head);
        if(type==="deliveries")this.renderDeliveryStatus(box);else this.renderProductionStatus(box);
        shade.append(box);document.body.append(shade);
    }
    renderDeliveryStatus(box){
        const deliveries=this.operationsOverview.openDeliveries();
        if(!deliveries.length){box.append(this.el("p","Keine offenen Lieferungen."));return;}
        for(const o of deliveries){const row=this.el("div");Object.assign(row.style,{border:"1px solid #ddd",borderRadius:"9px",padding:"11px",margin:"8px 0"});const material=this.label(o.material||o.itemId),qty=Number(o.quantity??o.amount)||0,unit=o.unit||"",supplier=o.supplierName||o.supplierId||"Lieferant",vehicle=(this.company.vehicles||[]).find(v=>String(v.id)===String(o.vehicleId)),transport=vehicle?`Eigener LKW: ${vehicle.name||vehicle.label||vehicle.id}`:(o.transportMode==="own"?"Eigener LKW":"Lieferant / Spedition"),eta=o.eta||o.plannedEta||o.arrivalAt||o.expectedAt;row.append(this.el("strong",`${supplier} · ${material}${qty?` · ${this.amount(qty)} ${unit}`:""}`),this.el("div",`Transport: ${transport}`),this.el("div",`Status: ${this.orderStatus(o.status)}`),this.el("div",`Restzeit: ${this.remaining(eta)}`));if(eta){const ts=this.validTimestamp(eta);if(ts)row.append(this.el("div",`Ankunft: ${new Date(ts).toLocaleString(i18n.locale)}`));}box.append(row);}
    }
    renderProductionStatus(box){
        const jobs=this.operationsOverview.activeProduction();
        if(!jobs.length){box.append(this.el("p","Keine laufende oder geplante Produktion."));return;}
        for(const j of jobs){const row=this.el("div");Object.assign(row.style,{border:"1px solid #ddd",borderRadius:"9px",padding:"11px",margin:"8px 0"});const product=j.recipe?.label||this.label(j.productId||j.product||j.recipeId),qty=j.plan?.output??j.quantity??j.output??j.batches??0,unit=j.recipe?.outputUnit||j.recipe?.unit||j.unit||"",finish=j.finishAt||j.completeAt;row.append(this.el("strong",`${product}${qty?` · ${this.amount(qty)} ${unit}`:""}`),this.el("div",`Status: ${this.productionStatus(j.status)}`));if(finish)row.append(this.el("div",`Restzeit: ${this.remaining(finish)}`));box.append(row);}
    }

    header(panel){const head=this.el("div");Object.assign(head.style,{display:"flex",justifyContent:"space-between",gap:"10px",alignItems:"center"});const title=this.el("div",`🏭 ${this.company.name||"WorldProject"} · Betrieb ${this.company.slotNo||1}`);Object.assign(title.style,{fontSize:"24px",fontWeight:800});head.append(title,this.button("✕",()=>this.close()));panel.append(head);}
    renderSetup(panel){const state=this.controller.getSetupState(this.company),progress=state.progress;const info=this.card("🛠️ Gründungsphase – Gebäude einrichten");info.append(this.el("div",`${state.building.name} · ${state.building.areaM2||0} m²`));info.append(this.small("Der Betrieb ist noch nicht eröffnet. Lieferanten, Produktion und Kundenaufträge werden erst nach der Mindestausstattung freigeschaltet."));info.append(this.el("div",`Fortschritt: ${progress.percent} %`));panel.append(info);}
    renderSuppliers(grid){const profile=this.controller.getIndustryProfile(this.company),suppliers=this.anchorCard(this.card(i18n.t("operations.supply_title")),"dashboard-deliveries"),items=profile.allowedItems||[];if(!items.length){suppliers.append(this.el("div","Für diese Branche wird der Lieferantenkatalog noch ergänzt."));grid.append(suppliers);return;}suppliers.append(this.small("Rohstoffe und Verpackungsmaterial bestellen."),this.button("Einkauf öffnen",()=>this.openOperationalSupplyChain("buy")));const openOrders=this.operationsOverview.openDeliveries();if(openOrders.length)suppliers.append(this.el("strong",`${openOrders.length} offene Lieferung${openOrders.length===1?"":"en"}`));else suppliers.append(this.small("Keine offenen Lieferungen."));grid.append(suppliers);}
    ...
    render(panel){
        ...
        if(profile.branchKey==="brewery"){production.append(this.small("Produktionsmengen und neue Chargen planen."),this.button("Produktion planen",()=>this.openOperationalSupplyChain("production")));const q=this.operationsOverview.activeProduction();if(q.length)production.append(this.el("strong",`${q.length} laufende/geplante Produktion${q.length===1?"":"en"}`));}else production.append(this.small("Branchenspezifische Produktionsrezepte werden noch ergänzt."));grid.append(production);
        const customerCard=this.anchorCard(this.card("📋 Kundenaufträge"),"dashboard-customer-orders"),customerOrders=this.operationsOverview.openCustomerOrders();if(!customerOrders.length)customerCard.append(this.small("Keine offenen Kundenaufträge."));for(const o of customerOrders){const qty=o.quantity??o.amount??0,done=o.delivered??o.deliveredQuantity??o.fulfilledQuantity??o.deliveredAmount??0,product=this.label(o.product||o.productId||o.itemId),customer=o.customerName||o.customer?.name||"Kunde";customerCard.append(this.el("strong",`${customer} · ${product}`),this.small(`${this.amount(done)} / ${this.amount(qty)} erfüllt · ${this.orderStatus(o.status)}`));}grid.append(customerCard);
        const fleet=this.card("🚛 Fuhrpark, Tank & Wartung");const first=this.company.vehicles?.[0];fleet.append(this.el("div",first?`${first.name||first.label||"Fahrzeug"} · ${this.vehicleStatus(first.status)} · ${Math.round(first.odometerKm||0)} km · Tank ${this.amount(first.fuelLiters)} l · Zustand ${Number(first.condition||100).toFixed(1)} %`:"Noch kein Fahrzeug"));grid.append(fleet);
        ...
        const finance=this.card("📊 Tages-/Wochenübersicht");finance.append(this.stockRow("Einnahmen 7 Tage",`${this.money(report.income)} €`),this.stockRow("Kosten 7 Tage",`${this.money(report.costs)} €`),this.stockRow("Gewinn 7 Tage",`${this.money(report.profit)} €`));const nav=this.el("div");Object.assign(nav.style,{display:"flex",gap:"6px",flexWrap:"wrap",marginTop:"10px"});nav.append(this.button(`🚚 ${operationCounters.deliveries} Lieferungen`,()=>this.openStatusOverlay("deliveries")),this.button(`🏗️ ${operationCounters.production} Produktion`,()=>this.openStatusOverlay("production")),this.button(`📋 ${operationCounters.customerOrders} Kundenaufträge`,()=>this.jumpTo("dashboard-customer-orders")));finance.append(nav);grid.append(finance);panel.append(grid);
    }
}
