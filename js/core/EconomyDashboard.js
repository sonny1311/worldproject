...
export class EconomyDashboard {
    ...
    renderSetup(panel){
        ...
        for(const item of state.equipmentCatalog){
            const owned=(state.building.equipment||[]).some(x=>(x.id||x)===item.id),c=this.card(`${item.required?"★ ":""}${item.name}`);
            c.append(this.small(`${this.money(item.price)} € · Bereich ${item.room}${item.required?" · Mindestausstattung":" · optional"}`));
            c.append(this.el("div",owned?"✅ Bereits installiert":"Noch nicht vorhanden"));
            if(!owned)c.append(this.button(`${this.money(item.price)} € kaufen & installieren`,async()=>{const r=this.controller.buySetupEquipment(this.company,item.id);alert(r.success?`${item.name} installiert.`:r.reason);if(r.success&&this.company.serverCompanyId)await window.worldAccounts?.authApi?.updateBusinessSetup?.(this.company.serverCompanyId,this.company.setupPhase,this.company.buildingState);this.render(panel);}));
            grid.append(c);
        }
        panel.append(grid);
        const finish=this.card("🚪 Betrieb eröffnen");
        finish.append(this.small(progress.complete?"Die Mindestausstattung ist vollständig.":`Es fehlt noch: ${progress.missing.join(", ")||"Mindestausstattung ist für diese Branche noch nicht definiert."}`));
        finish.append(this.button("Betrieb eröffnen",async()=>{const r=this.controller.startBusinessOperations(this.company);alert(r.success?"Betrieb eröffnet. Einkauf, Produktion und Aufträge sind jetzt freigeschaltet.":r.reason);if(r.success&&this.company.serverCompanyId)await window.worldAccounts?.authApi?.updateBusinessSetup?.(this.company.serverCompanyId,this.company.setupPhase,this.company.buildingState);this.render(panel);}));panel.append(finish);
    }

    renderSuppliers(grid){
        const profile=this.controller.getIndustryProfile(this.company),suppliers=this.card("📦 Lieferanten – selbst auswählen");
    ...
    render(panel){
        ...
        const production=this.card("🏗️ Produktion");
        ...
        const finance=this.card("📊 Tages-/Wochenübersicht");finance.append(this.stockRow("Einnahmen 7 Tage",`${this.money(report.income)} €`),this.stockRow("Kosten 7 Tage",`${this.money(report.costs)} €`),this.stockRow("Gewinn 7 Tage",`${this.money(report.profit)} €`),this.small(`${report.openSupplierOrders} offene Lieferungen · ${report.queuedProduction} Produktionen · ${report.openCustomerOrders} Kundenaufträge`));grid.append(finance);
