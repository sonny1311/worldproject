...
export class AdminWorkspaceController{
 constructor({control=null,dashboard=null,audit=null}={}){this.control=control||window.worldAdminControl;this.dashboard=dashboard||window.worldAdminDashboard;this.audit=audit||window.worldAdminAudit;this.state={section:'overview',query:'',selected:null,pendingAction:null,lastResult:null};}
 ...
 preview(action,payload={}){const risk=['money','inventory','ban','delete','world','feature_flag','premium','coins'].some(k=>String(action).includes(k))?'high':'normal';this.state.pendingAction={action,payload,risk,createdAt:Date.now(),requiresConfirmation:risk==='high',requiresReason:risk==='high'};return this.state.pendingAction;}
 cancelPending(){this.state.pendingAction=null;return this.view();}
 async execute({confirmed=false,reason=''}={}){const p=this.state.pendingAction;if(!p)throw new Error('Keine Admin-Aktion vorbereitet');if(p.requiresConfirmation&&!confirmed)throw new Error('Bestätigung erforderlich');if(p.requiresReason&&!String(reason||'').trim())throw new Error('Begründung für kritische Admin-Aktion erforderlich');if(!this.control?.execute)throw new Error('Admin-Control nicht verfügbar');const result=await this.control.execute(p.action,{...p.payload,reason:String(reason||'').trim()});this.state.pendingAction=null;this.state.lastResult={action:p.action,result,at:Date.now()};return result;}
