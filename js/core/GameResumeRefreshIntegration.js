...
export function refreshTimedSystemsOnResume({force=false,now=Date.now()}={}){
 ...
 try{const company=window.worldPlayerCompany;if(company)runOperationalConsistency(company,now);}catch(error){console.warn('Betriebs-Konsistenz nach Rueckkehr fehlgeschlagen',error);}
