...
if(!proto.__worldSummaryNavigationIntegrated){
 ...
 proto.render=function(panel){
  ...
  const cards=[...panel.querySelectorAll('div')].filter(el=>el.children?.length>=2&&el.firstElementChild&&['💶 Firmenkonto','🪙 Coins','🚚 Fuhrpark','🏬 Lager','📦 Fertigwaren','📈 Wochengewinn','🎯 Aufgabe'].includes(el.firstElementChild.textContent?.trim()));
  ...
  for(const card of cards){
   const title=card.firstElementChild.textContent?.trim();Object.assign(card.style,{cursor:'pointer',transition:'transform .12s ease, box-shadow .12s ease'});card.title=card.title||'Klicken für Details';card.tabIndex=0;
   const run=()=>{
    if(title==='💶 Firmenkonto'||title==='📈 Wochengewinn')return jump(finance);
    if(title==='🚚 Fuhrpark')return jump(fleet);
    if(title==='🏬 Lager'||title==='📦 Fertigwaren')return jump(warehouse);
    if(title==='🪙 Coins'){
