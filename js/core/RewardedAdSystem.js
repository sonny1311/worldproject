...
const ts=v=>v instanceof Date?v.getTime():num(v);
...
export class RewardedAdSystem{
 constructor({config=RewardedAdConfig}={}){this.config=config;}
  ...
  const s=company.rewardedAds;
  s.history=Array.isArray(s.history)?s.history:[];s.revenueEvents=Array.isArray(s.revenueEvents)?s.revenueEvents:[];
  if(s.day!==dayKey(now)){s.day=dayKey(now);s.generalWatched=0;}
  return s;
 }
 generalState(company,now=Date.now()){
  const s=this.ensure(company,now),watched=Math.max(0,Math.min(this.config.dailyGeneralAds,num(s.generalWatched))),remaining=Math.max(0,this.config.dailyGeneralAds-watched);
  return{day:s.day,watched,remaining,total:this.config.dailyGeneralAds,complete:remaining===0};
 }
 registerProviderRevenue(company,{provider=null,placement=null,revenue=null,currency=null,providerEventId=null,now=Date.now()}={}){
  const s=this.ensure(company,now);const event={provider:String(provider||'unknown'),placement:String(placement||'unknown'),revenue:Number.isFinite(Number(revenue))?Number(revenue):null,currency:currency?String(currency):null,providerEventId:providerEventId?String(providerEventId):null,at:now};s.revenueEvents.push(event);if(s.revenueEvents.length>500)s.revenueEvents.splice(0,s.revenueEvents.length-500);return event;
 }
 confirmGeneralAd(company,{providerReceipt=null,now=Date.now()}={}){
  const state=this.generalState(company,now);if(state.complete)throw new Error('Das heutige Werbekontingent ist bereits ausgeschöpft');if(!providerReceipt?.completed)throw new Error('Werbung wurde nicht vollständig bestätigt');
  const s=this.ensure(company,now);s.generalWatched=state.watched+1;s.history.push({type:'general',placement:'home',at:now,receiptId:providerReceipt.id||null});
  if(providerReceipt.revenue!==undefined)this.registerProviderRevenue(company,{provider:providerReceipt.provider,placement:'home',revenue:providerReceipt.revenue,currency:providerReceipt.currency,providerEventId:providerReceipt.id,now});
  return{success:true,state:this.generalState(company,now),reward:this.config.generalReward};
 }
 resolveTimer(job){
  if(!job)return null;
 ...
 confirmTimeAd(company,job,{kind='job',providerReceipt=null,now=Date.now()}={}){
  ...
  if(providerReceipt.revenue!==undefined)this.registerProviderRevenue(company,{provider:providerReceipt.provider,placement:`time:${kind}`,revenue:providerReceipt.revenue,currency:providerReceipt.currency,providerEventId:providerReceipt.id,now});
  return{success:true,reductionMs:reduction,remainingMs:Math.max(0,next-now),state:this.timeAdState(job),timerKey:timer.key};
 }
}
